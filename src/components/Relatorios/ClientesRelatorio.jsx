...
const GerarRelatorio = ({ clientes, loading }) => {
  ...
  // Filtrar, ordenar e mapear clientes com base nas colunas selecionadas
  const prepararDadosRelatorio = () => {
    // Filtrar clientes com base no formato do relatório
    let dadosFiltered = clientes;
    if (reportFormat === 'ativos') {
      dadosFiltered = clientes.filter((cliente) => cliente?.status?.toLowerCase() === 'ativo');
    } else if (reportFormat === 'inativos') {
      dadosFiltered = clientes.filter((cliente) => cliente?.status?.toLowerCase() === 'inativo');
    }

    // Ordenar os clientes pelo nome (campo "nome")
    dadosFiltered.sort((a, b) => (a.nome || '').localeCompare(b.nome || ''));

    // Mapear apenas as colunas selecionadas
    return dadosFiltered.map((cliente) => {
      const dadosRelatorio = {};
      allColumns.forEach((col) => {
        if (selectedColumns[col.key]) {
          dadosRelatorio[col.label] = cliente[col.key] || 'N/A';
        }
      });
  ...
  const gerarRelatorioPDF = () => {
    ...
    if (dadosRelatorio.length === 0) {
      alert('Não há clientes para gerar o relatório.');
      return;
    }
  ...
  return (
    <>
      <Button
        ...
        disabled={loading || !clientes || clientes.length === 0}
        endIcon={<ConfigIcon />}
      >
        Gerar Relatório
      </Button>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <FormControl fullWidth>
              ...
              >
                <MenuItem value="completo">Todos os Clientes</MenuItem>
                <MenuItem value="ativos">Apenas Clientes Ativos</MenuItem>
                <MenuItem value="inativos">Apenas Clientes Inativos</MenuItem>
              </Select>
            </FormControl>
